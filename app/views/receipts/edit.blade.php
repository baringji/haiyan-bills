...
@section('title')
<title>Update receipt | {{ Lang::get('common.title') }}</title>
@stop
@section('meta')
  ...
  <section class="content">
    <div class="row">
      <div class="col-lg-12 col-xs-12">
        {{ Form::open(['route' => array('receipts.update', $receipt->id), 'method' => 'put', 'class' => 'form-horizontal', 'role' => 'form']) }}
          <div class="form-group{{ ($errors->first('bill')) ? ' has-error' : '' }}">
            ...
            <div class="col-xs-12 col-sm-5">
              <select class="form-control" name="bill" id="bill">
                ...
                <option value="{{ $bill->id }}" {{ ($bill->id == $receipt->bill_id) ? 'selected="selected"' : '' }}> {{ $bill->name }} </option>
                @endforeach
                @endif
              </select>
            </div>
          ...
          <div class="form-group{{ ($errors->first('amount')) ? ' has-error' : '' }}">
            ...
            <div class="col-xs-12 col-sm-5">
              <input class="form-control" type="text" name="amount" id="amount" placeholder="Amount" value="{{{ $receipt->amount or null }}}" />
            </div>
